...
app.post('/challenges', (req, res) => {
  ...
  try {
    ...
    db.run(
      'INSERT INTO challenges (title, detail, author) VALUES (?, ?, ?)',
      [title, detail, author],
      function (err) {
        if (err) {
          return res
            .status(500)
            .json({ message: '챌린지 글 작성 실패', error: err.message });
        }
        res
          .status(201)
          .json({ message: '챌린지 글 작성 성공', challengeId: this.lastID });
      }
    );
  } catch (error) {
    res.status(401).json({ message: '유효하지 않은 토큰' });
  ...
  db.all(
    ...
    (err, rows) => {
      if (err) {
        return res
          .status(500)
          .json({ message: '챌린지 글 목록 조회 실패', error: err.message });
      }
      res.json(rows);
    }
  );
